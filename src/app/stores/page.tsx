...
export default function StoresPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (!res.ok) {
      toast({
        title: json.error ?? "Failed to create store",
        variant: "destructive",
      });
      return;
    }
  ...
  async function handleRename(storeId: string) {
    ...
    if (!res.ok) {
      toast({
        title: json.error ?? "Failed to rename",
        variant: "destructive",
      });
      return;
    }
    setStores((prev) =>
      prev.map((s) => (s.id === storeId ? { ...s, name: json.data.name } : s)),
    );
    setRenamingId(null);
  ...
  async function handleDelete() {
    ...
    if (!res.ok) {
      toast({
        title: json.error ?? "Failed to delete",
        variant: "destructive",
      });
      return;
    }
